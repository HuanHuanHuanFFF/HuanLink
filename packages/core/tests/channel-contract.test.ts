...
describe("Channel Contract", () => {
  ...
  test.each(["not-a-time", "2026-07-22", "2026-02-30T00:00:00.000Z"])(
    "rejects an invalid inbound receivedAt %s",
    (receivedAt) => {
      expect(() =>
  ...
  test.each(["2026-07-22T00:00:00Z", "2026-07-22T00:00:00.123Z"])(
    "accepts a valid UTC ISO-8601 receivedAt %s",
    (receivedAt) => {
      expect(() =>
        assertValidInboundChannelMessage(inbound({ receivedAt })),
      ).not.toThrow();
  ...
  test("isolates sessions by channel, conversation kind, conversation, and thread", () => {
    ...
    expect(channelSessionIdFor(route({ channelId: "qq-secondary" }))).not.toBe(
      group,
    );
    expect(channelSessionIdFor(route({ conversationKind: "direct" }))).not.toBe(
      group,
    );
    expect(channelSessionIdFor(route({ conversationId: "20001" }))).not.toBe(
      group,
    );
    expect(channelSessionIdFor(route({ threadId: "thread:one" }))).toBe(
      "channel:qq-main:group:20000:thread:thread%3Aone",
    );
  });
  ...
  test.each(["D:\\workspace\\result.txt", "/var/lib/huanlink/result.txt"])(
    "accepts an absolute local attachment path %s",
    (path) => {
      expect(() =>
        assertValidOutboundChannelMessageParts([
          {
