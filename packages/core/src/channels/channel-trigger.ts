import type { ChannelTrigger } from "./channel-message.js";

/** Adapter 从平台消息中提取、供 Channel 判断触发原因的临时事实。 */
export type ChannelTriggerSignals = {
...
const CHANNEL_COMMAND_PATTERN = /^\/[\p{L}\p{N}_-]+(?=\s|$)/u;
...
export function resolveChannelTrigger(
  signals: ChannelTriggerSignals,
): ChannelTrigger | undefined {
  const leadingText = signals.leadingText?.trimStart();
  if (leadingText !== undefined && CHANNEL_COMMAND_PATTERN.test(leadingText)) {
    return { kind: "command" };
  }
