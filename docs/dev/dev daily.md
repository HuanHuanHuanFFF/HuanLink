...
## 7.6
修改项目名称Huaness→HuanLink 幻联
修改README.md,以及仓库,并确定一些初期定位和需求
彻底转型
## 7.7
初步对接openai-agents-js核心
## 7.8
取消huaness在代码里的痕迹,全部改为huanlink
## 7.9
## demo first
先大干快上做demo,后续再慢慢优化写模块,不然每次review看不懂,也不知道核心痛点是啥,怎么优化也想不到

## 7.11
推进demo计划phase 0,1,2
## codex A2A adapter
- **A2A v1.0 标准服务端主链路已接入**：不是自定义 A2A-like，也没有启用 v0.3 兼容模式。
- 已支持 Agent Card、`SendMessage`、流式执行、`GetTask`、`ListTasks`、任务订阅、取消、标准状态和 Artifact。
- 这套链路已经通过真实 Codex 修改代码验证。
## 7.12
推进demo
完成:
- phase 2 接入 接入真实 Codex app-server
- phase 3 HuanLink 原生接入 A2A
## 7.13
推进demo,并完成 QQ派发任务→HuanLink→codex→执行任务→结果返回HuanLink→QQ回复告知 的测试
后续测试出现bug,正在修复中,同时补全日志功能,接入Vercel AI SDK
## 初步测试
![[HuanLink demo测试0.png]]![[HuanLink demo测试1.png]]![[HuanLink demo测试2.png]]

## 7.14
phase 4测试成功
真实 QQ → DeepSeek MainAgent → AgentCall → A2A →Codex app-server → Artifact → 最新上下文回流
并且异步任务中能和agent对话,可查询异步任务执行情况
## 7.15
前期demo已全部完成,成功验收,具体可见[[23-a2a-first-real-demo-plan]]
已完成当前5个阶段的全部开发任务,接下来将进入正式开发阶段

## 7.19
### 权限控制怎么做
codex那边当前的A2A是默认当成用户的身份,如果主控agent通过A2A发给外部agent干活的时候,如果有危险的操作,怎么确定是不是用户确认的需求,而不是主控agent的幻觉/理解错误
我希望能最大程度的智能化,像codex的approve for me,能让用户用自然语言受权,同时也有工程兜底,不要太复杂的权限管理
### 权限控制及任务/会话管理
太难了,难怪没人做A2A
我想要像approve for me那样智能化的方案,但是主控agent和调用的远端agent的上下文,项目理解都不一样,甚至不在同一台机器上,要干活派任务,怎么派到合适的远端session?怎么确保项目理解正确,续派任务的时候不知道是哪个会话,会话要不要刷新上下文?结合当前背景等等
有点超出我的工程能力了
...
## 7.25
补充设计通用channel模块边界
主要是希望逻辑简单,不复杂,安全上没有大问题,只负责转发到session,不负持久化,把操作交给agent,不要做过多的安全审计,附件缓存,链接给agent就行
## 7.28
推进D11-B04,B05
## 7.29
推进B05,并修订需求,确定 command>mention
## 8.3
推进到B07,确定了很多设计
