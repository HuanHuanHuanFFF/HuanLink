...
- [x] 在 `apps/server` 精确固定 `@openai/agents-extensions@0.12.0`、`ai@6.0.224`、`@ai-sdk/deepseek@2.0.47`。
- [x] 执行 `corepack.cmd pnpm install`，确认 lockfile 只包含预期依赖变化。
- [x] 执行 `corepack.cmd pnpm --filter @huanlink/server typecheck`，记录接入代码尚未使用依赖但依赖图可解析。
- [x] 单独提交：`build(server): 添加 AI SDK DeepSeek 依赖`。
...
- [x] RED：先为 DeepSeek 默认 provider/model/base URL、缺失 Key、非 DeepSeek provider、非 HTTPS base URL 和 Key 不泄漏写测试。
- [x] 运行 `corepack.cmd pnpm --filter @huanlink/server test -- tests/runtime-config.test.ts`，确认测试因模型配置尚不存在而按预期失败。
- [x] GREEN：新增 `MainAgentModelConfig` 和启动时解析；`loadPhase4QqRuntimeConfigFromEnv()` 返回 `mainAgentModel`。
- [x] 将 `.env.example` 改为以下合同，并用同一合同更新被 Git 忽略的本地 `.env`：
...
- [x] 重跑聚焦测试，确认通过；检查 `git status --short --ignored`，确保 `.env` 不在待提交文件中。
- [x] 单独提交：`feat(server): 添加 MainAgent 模型配置`。
...
- [x] RED：用自定义 fake `fetch` 捕获 DeepSeek 请求；通过真实 AI SDK provider、真实 bridge、真实 OpenAI Agents Runner 和现有 `submit_codex_agent_call` 工具完成“工具调用后返回文本”的两次响应。
- [x] 断言请求 URL 为 `/beta/chat/completions`、模型为 `deepseek-v4-flash`、`thinking.type` 为 `disabled`、函数工具含 `strict: true`，并断言 fake `AgentCallInvoker` 收到任务。
- [x] 运行 `corepack.cmd pnpm --filter @huanlink/server test -- tests/main-agent-model.test.ts`，确认因模型绑定尚不存在而失败。
- [x] GREEN：用 `createDeepSeek` 创建模型；用 `wrapLanguageModel` 的 `transformParams` 只为函数工具恢复 `strict: true`；用 `aisdk(...)` 返回现有 Agent 可用的模型及 provider settings。
- [x] 为 `createPhase3MainAgentRuntime` 增加可选 `modelBinding`，默认值仅保留给现有注入 Runner 的测试；不得复制工具或 Runner。
- [x] 重跑聚焦测试和 `corepack.cmd pnpm --filter @huanlink/server typecheck`。
- [x] 单独提交：`feat(server): 创建 DeepSeek AI SDK 模型绑定`。
...
- [x] 单独提交：`test(server): 添加真实 DeepSeek 工具调用 smoke`。

## Task 6：全仓验证与 Phase 4 闸门

- [x] 执行 `corepack.cmd pnpm test`。
- [x] 执行 `corepack.cmd pnpm typecheck`。
- [x] 执行 `corepack.cmd pnpm build`。
- [x] 执行 `git diff --check`，并审阅 `git status --short`、`git diff --stat` 和待推送提交列表。
- [x] 确认 `.env`、API Key、QQ 群号和 access token 均未进入提交或测试输出。
