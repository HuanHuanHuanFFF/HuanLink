...
```ts
executionMode: "background" | "wait"
```

- `background`：创建任务并快速返回 `accepted + taskId`；MainAgent 看到回执后继续当前 turn。任务进入终态后，再触发新的 MainAgent turn。
- `wait`：创建同样的任务，但当前 MainAgent turn 等到终态并直接使用最终结果。它不能阻塞服务进程、其他会话或其他后台任务。
- `wait` 的终态只交给当前 turn 消费，不再额外触发终态回流；`background` 才在终态触发新的 turn。
- 每个 Tool 声明默认模式。长耗时 Tool 默认 `background`，快速普通 Tool 继续直接等待。
- 用户明确要求“等待完成后再继续”时，MainAgent 可以覆盖 Tool 的默认模式。
- v1.0 不根据预计耗时自动猜测模式。

`executionMode` 是 HuanLink 的调用控制信息，不属于具体 Executor 的业务参数。
