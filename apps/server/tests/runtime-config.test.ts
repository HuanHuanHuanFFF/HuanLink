...
describe("server runtime config public surface", () => {
  test("does not export the legacy RuntimeConfig loader", () => {
    expect(server).not.toHaveProperty("loadRuntimeConfigFromEnv");
  });

  test("does not advertise legacy RuntimeConfig keys in the root .env.example", async () => {
    const envExample = await readFile(
      new URL("../../../.env.example", import.meta.url),
      "utf8"
    );

    expect(envExample).not.toMatch(/^HUANLINK_EVENT_LOG_BASE_DIR=/m);
    expect(envExample).not.toMatch(
      /^HUANLINK_EVENT_LOG_NEXT_SEQ_CACHE_SIZE=/m
    );
    expect(envExample).not.toMatch(/^HUANLINK_AGENT_DEFAULT_MAX_STEPS=/m);
    expect(envExample).toMatch(/^HUANLINK_LOG_LEVEL=/m);
  });
});
