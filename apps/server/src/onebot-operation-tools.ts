...
const standardRequest = z.discriminatedUnion("operation", [
  ...
  z
    .object({
      ...
      params: z.object({ groupId: positiveId, nodes: z.array(forwardNode).min(1) }).strict()
    })
    .strict(),
  z
    .object({
      operation: z.literal("sendPrivateForwardMessage"),
      params: z.object({ userId: positiveId, nodes: z.array(forwardNode).min(1) }).strict()
    })
    .strict(),
  ...
  z.object({ operation: z.literal("canSendRecord"), params: noParams }).strict(),
  z.object({ operation: z.literal("getStrangerInfo"), params: z.object({ userId: positiveId, noCache: z.boolean().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("getFriendList"), params: noParams }).strict(),
  z.object({ operation: z.literal("getGroupInfo"), params: z.object({ groupId: positiveId, noCache: z.boolean().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("getGroupList"), params: noParams }).strict(),
  z.object({ operation: z.literal("getGroupMemberInfo"), params: z.object({ groupId: positiveId, userId: positiveId, noCache: z.boolean().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("getGroupMemberList"), params: z.object({ groupId: positiveId }).strict() }).strict(),
  z.object({ operation: z.literal("getGroupHonorInfo"), params: z.object({ groupId: positiveId, type: z.enum(["talkative", "performer", "legend", "strong_newbie", "emotion", "all"]) }).strict() }).strict(),
  z.object({ operation: z.literal("sendLike"), params: z.object({ userId: positiveId, times: z.number().int().min(1).max(10).optional() }).strict() }).strict(),
  z.object({ operation: z.literal("uploadGroupFile"), params: z.object({ groupId: positiveId, path: nonBlank, name: nonBlank.optional() }).strict() }).strict(),
  z.object({ operation: z.literal("uploadPrivateFile"), params: z.object({ userId: positiveId, path: nonBlank, name: nonBlank.optional() }).strict() }).strict()
]);

const privilegedRequest = z.discriminatedUnion("operation", [
  z.object({ operation: z.literal("deleteMessage"), params: z.object({ messageId }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupKick"), params: z.object({ groupId: positiveId, userId: positiveId, rejectAddRequest: z.boolean().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupBan"), params: z.object({ groupId: positiveId, userId: positiveId, durationSeconds: z.number().int().min(0) }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupWholeBan"), params: z.object({ groupId: positiveId, enabled: z.boolean() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupAdmin"), params: z.object({ groupId: positiveId, userId: positiveId, enabled: z.boolean() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupCard"), params: z.object({ groupId: positiveId, userId: positiveId, card: z.string() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupName"), params: z.object({ groupId: positiveId, name: nonBlank }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupLeave"), params: z.object({ groupId: positiveId, dismiss: z.boolean().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupSpecialTitle"), params: z.object({ groupId: positiveId, userId: positiveId, title: z.string(), durationSeconds: z.number().int().min(-1).optional() }).strict() }).strict(),
  z.object({ operation: z.literal("setFriendAddRequest"), params: z.object({ flag: nonBlank, approve: z.boolean(), remark: z.string().optional() }).strict() }).strict(),
  z.object({ operation: z.literal("setGroupAddRequest"), params: z.object({ flag: nonBlank, subType: z.enum(["add", "invite"]), approve: z.boolean(), reason: z.string().optional() }).strict() }).strict()
]);
...
type ToolResult =
  | ConversationJsonValue
  | { readonly status: "error" | "uncertain"; readonly tool: ToolName; readonly error: string };
...
export type OneBot11OperationTools = ReturnType<typeof createOneBot11OperationTools>;
...
export function createOneBot11OperationTools(
  ...
  const standard = tool<typeof standardParameters, OpenAiAgentsRunContext>({
    ...
    strict: true,
    isEnabled: ({ runContext }) => isExternalSession(options, runContext.context.sessionId),
    errorFunction: (_context, error) => JSON.stringify(errorResult(ONEBOT11_STANDARD_TOOL_NAME, error)),
...
async function executeToolCall<Input extends StandardToolInput | PrivilegedToolInput>(input: {
  options: CreateOneBot11OperationToolsOptions;
  logger: RuntimeLogger;
  toolName: ToolName;
  input: Input;
  ...
  if (context === undefined) {
    return JSON.stringify(errorResult(input.toolName, new Error("OneBot Tool requires a HuanLink RunContext")));
  }
  if (!isExternalSession(input.options, context.sessionId)) {
    return JSON.stringify(errorResult(input.toolName, new Error(`Session ${context.sessionId} is not an external Channel session`)));
  }
  if (input.toolCallId === undefined || input.toolCallId.trim().length === 0) {
    return JSON.stringify(errorResult(input.toolName, new Error("OneBot Tool requires the SDK Tool Call ID")));
  }
  ...
  try {
    input.options.sessions.appendAgentToolCall(context.sessionId, {
      ...
      toolName: input.toolName,
      arguments: input.input as unknown as Readonly<Record<string, ConversationJsonValue>>
    });
  } catch (error) {
    toolLogger.error("onebot.operation.tool_call_record_failed", { errorType: errorType(error) });
  ...
  if (operations === undefined) {
    return complete(errorResult(input.toolName, new Error(`No OneBot Operations are registered for ${input.input.channelId}`)));
  }
  ...
  try {
    ...
    if (value === undefined) {
      return complete(errorResult(input.toolName, new Error("OneBot operation returned non-JSON data")));
    }
...
async function executeStandard(
  ...
  switch (request.operation) {
    case "sendGroupMessage":
      return await executeSend(options, groupRoute(input.channelId, request.params.groupId), () => operations.standard.sendGroupMessage(request.params));
    case "sendPrivateMessage":
      return await executeSend(options, directRoute(input.channelId, request.params.userId), () => operations.standard.sendPrivateMessage(request.params));
    case "sendGroupForwardMessage":
      return await executeSend(options, groupRoute(input.channelId, request.params.groupId), () => operations.standard.sendGroupForwardMessage(request.params));
    case "sendPrivateForwardMessage":
      return await executeSend(options, directRoute(input.channelId, request.params.userId), () => operations.standard.sendPrivateForwardMessage(request.params));
    case "getMessage":
      return { data: await operations.standard.getMessage(request.params) };
    case "getForwardMessage":
      return { data: await operations.standard.getForwardMessage(request.params) };
    case "getLoginInfo": return { data: await operations.standard.getLoginInfo() };
    case "getVersionInfo": return { data: await operations.standard.getVersionInfo() };
    case "getStatus": return { data: await operations.standard.getStatus() };
    case "canSendImage": return { data: await operations.standard.canSendImage() };
    case "canSendRecord": return { data: await operations.standard.canSendRecord() };
    case "getStrangerInfo":
      assertAllowed(options, directRoute(input.channelId, request.params.userId));
      return { data: await operations.standard.getStrangerInfo(request.params) };
    // 用户已将无显式目标的账号级只读枚举归入 standard；它们不伪造一个
    // route，也不改变带明确群/私聊目标操作必须经过 assertAllowed 的规则。
    case "getFriendList": return { data: await operations.standard.getFriendList() };
    case "getGroupInfo":
      assertAllowed(options, groupRoute(input.channelId, request.params.groupId));
      return { data: await operations.standard.getGroupInfo(request.params) };
    case "getGroupList": return { data: await operations.standard.getGroupList() };
    case "getGroupMemberInfo":
      assertAllowed(options, groupRoute(input.channelId, request.params.groupId));
      return { data: await operations.standard.getGroupMemberInfo(request.params) };
    case "getGroupMemberList":
      assertAllowed(options, groupRoute(input.channelId, request.params.groupId));
      return { data: await operations.standard.getGroupMemberList(request.params) };
    case "getGroupHonorInfo":
      assertAllowed(options, groupRoute(input.channelId, request.params.groupId));
      return { data: await operations.standard.getGroupHonorInfo(request.params) };
    case "sendLike":
      return {
    ...
    case "uploadGroupFile":
      assertAllowed(options, groupRoute(input.channelId, request.params.groupId));
      return { data: await options.runOutbound(groupRoute(input.channelId, request.params.groupId), () => operations.standard.uploadGroupFile(request.params)) };
    case "uploadPrivateFile":
      assertAllowed(options, directRoute(input.channelId, request.params.userId));
      return { data: await options.runOutbound(directRoute(input.channelId, request.params.userId), () => operations.standard.uploadPrivateFile(request.params)) };
  }
}
...
async function executePrivileged(
  ...
  const request = input.request;
  switch (request.operation) {
    case "deleteMessage": return await operations.privileged.deleteMessage(request.params);
    case "setGroupKick": return await operations.privileged.setGroupKick(request.params);
    case "setGroupBan": return await operations.privileged.setGroupBan(request.params);
    case "setGroupWholeBan": return await operations.privileged.setGroupWholeBan(request.params);
    case "setGroupAdmin": return await operations.privileged.setGroupAdmin(request.params);
    case "setGroupCard": return await operations.privileged.setGroupCard(request.params);
    case "setGroupName": return await operations.privileged.setGroupName(request.params);
    case "setGroupLeave": return await operations.privileged.setGroupLeave(request.params);
    case "setGroupSpecialTitle": return await operations.privileged.setGroupSpecialTitle(request.params);
    case "setFriendAddRequest": return await operations.privileged.setFriendAddRequest(request.params);
    case "setGroupAddRequest": return await operations.privileged.setGroupAddRequest(request.params);
  }
}
...
async function executeSend(
  ...
  if (id === undefined) {
    throw new OneBot11DeliveryUncertainError("OneBot send response did not contain message_id");
  }
...
function groupRoute(channelId: string, groupId: string): ChannelConversationRouteV1 {
  return { channelId, conversationKind: "group", conversationId: groupId };
}

function directRoute(channelId: string, userId: string): ChannelConversationRouteV1 {
...
function isExternalSession(
  ...
  return options.sessions.getSessionMetadata(sessionId)?.kind === "external_channel";
}
...
function asConversationJsonValue(value: unknown): ConversationJsonValue | undefined {
  try {
...
function errorResult(tool: ToolName, error: unknown): Extract<ToolResult, { status: "error" | "uncertain" }> {
  return {
...
function isErrorResult(value: ToolResult): value is Extract<ToolResult, { status: "error" | "uncertain" }> {
  return (
    typeof value === "object" &&
    value !== null &&
    "status" in value &&
...
function isDeliveryUncertain(error: unknown): boolean {
  return error instanceof OneBot11DeliveryUncertainError ||
    (error instanceof ChannelOperationError && error.code === "delivery_uncertain");
}
